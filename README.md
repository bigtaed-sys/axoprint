...
## 5. Веб-печать из браузера (с телефона/любого ПК)

Релей может отдавать страницу печати прямо по домену: заходишь на `https://<домен>`,
вводишь пароль, выбираешь принтер и файл (PDF/JPG/PNG) → задание уходит в очередь → агент печатает.

Включается паролем при деплое:
```bash
grep AXO__TOKEN /etc/systemd/system/axoprint-relay.service   # текущий токен
cd ~/axoprint && git pull
sudo AXO_WEBPASS='ВАШ_ПАРОЛЬ' bash deploy/install.sh mona-tech.ru <ТОТ_ЖЕ_ТОКЕН>
```
Без `AXO_WEBPASS` веб-панель выключена (по `/` отдаётся текстовый статус). Пароль хранится
в systemd-юните; при повторном запуске `install.sh` без `AXO_WEBPASS` прежний пароль сохраняется.
Доступ — по HTTPS, cookie HttpOnly/Secure.
